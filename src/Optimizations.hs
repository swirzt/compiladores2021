...
optimizer (BinaryOp info bOp tm1 tm2) = do tm1' <- optimizer tm1
                                           tm2' <- optimizer tm2
                                           return $ BinaryOp info bOp tm1' tm2' 
optimizer (Fix info fName fTy vName vTy tm) = do tm' <- optimizer tm
                                                 return $ Fix info fName fTy vName vTy tm' 
optimizer (IfZ info tmb tmt tmf) = do  tmb' <- optimizer tmb
                                       tmt' <- optimizer tmt
                                       tmf' <- optimizer tmf
                                       return $ IfZ info tmb' tmt' tmf'
optimizer (Let info name ty tm1 tm2) = do  tm1' <- optimizer tm1
                                           tm2' <- optimizer tm2
