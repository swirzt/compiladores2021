...
fd4Main xs =
  pretty "nat fd4main()"
    <+> braces
      (nest 2 (line <> vsep (vals2doc xs ++ [pretty "return 0;"])) <> line)
  where
    vals2doc :: [IrDecl] -> [Doc a]
    vals2doc [] = []
    vals2doc [IrVal n t _] =
      [name n <+> pretty "=" <+> ir2doc t <> semi, irPrintN (name n), semi]
    vals2doc (IrVal n t _ : ds) =
      (name n <+> pretty "=" <+> ir2doc t <> semi) :
      vals2doc ds
...
stmts :: [Doc a] -> Doc a
stmts xs =
  parens $ braces $ foldr (\x ds -> nest 2 (line <> x <> semi) <> ds) line xs

ir2doc :: Ir -> Doc a
ir2doc (IrVar n) = name n
ir2doc (IrGlobal n) = name n
ir2doc (IrCall f args codomi) =
  parens
    ( parens (ty2Doc codomi <+> pretty "(*)" <+> tupled (argsTy2Doc args))
        <+> ir2doc f
    )
    <> tupled (map (\(ir, _) -> ir2doc ir) args)
...
ir2doc (IrIfZ c a b) =
  sep
    [ir2doc c, nest 2 (pretty "?" <+> ir2doc b), nest 2 (colon <+> ir2doc a)]
ir2doc (IrPrint str t) =
  stmts
    [ pretty "wprintf" <> parens (pretty "L" <> pretty (show str)),
      irPrintN (ir2doc t)
    ]
ir2doc (MkClosure f args) =
  pretty "fd4_mkclosure"
...
ir2C :: IrDecls -> String
ir2C (IrDecls xs) =
  unpack . renderStrict . layoutSmart defaultLayoutOptions $
    vsep (prelude : mempty : map decl2doc xs ++ [mempty, mempty, fd4Main xs])